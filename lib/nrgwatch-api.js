...
class NRGWatchApi extends BaseClient {
  ...
  static ENDPOINTS = {
    ...
    V2_OTA: 'api/v2/ota',
    V2_WPU_OUTSIDE_TEMP: 'api/v2/wpu/outside_temp',
  };
  ...
  setSettings(host, username, password, isAuthenticated, enableVirtualRemote, virtualRemoteIndex, useApiV2 = false) {
    ...
    this.webclient._userName = typeof username === 'string' ? username.trim() : username;
    this.webclient._passWord = typeof password === 'string' ? password.trim() : password;
    ...
    try {
      if (this.webclient._useApiV2) {
        ...
        if (result.status === 'success' && result.data) {
          // Firmware variants may return the payload as data directly or wrapped in data.ithostatus.
          if (result.data.ithostatus && typeof result.data.ithostatus === 'object') {
    ...
    try {
      if (this.webclient._useApiV2) {
        ...
        if (result.status === 'success' && result.data != null) {
          // data may be a number directly or an object with a currentspeed field
          return typeof result.data === 'object'
            ? (result.data.currentspeed ?? result.data.speed ?? result.data)
            : result.data;
        }
      ...
      if (result.data?.currentspeed != null) {
  ...
  /**
   * Retrieves detailed speed info including add-on tracked timer fields.
   * Requires REST API v2 (firmware 3.1.4+ for the timer fields).
    * @returns {Promise<{currentspeed?: number, speed?: number, timer_remaining_ms?: number, timer_speed?: number}>}
   * @throws {Error} If the request fails, returns invalid data, or v2 is disabled
   */
  async getSpeedInfo() {
    if (!this.webclient._useApiV2) {
      throw new Error('getSpeedInfo requires REST API v2 (firmware 3.0.0+)');
    }
    ...
    try {
      ...
      if (result.status === 'success' && result.data != null) {
        ...
        return { speed: result.data };
      }
      throw new Error('Invalid v2 response format: missing speed data');
    } catch (error) {
      this.homey?.error('Failed to get speed info:', error.message);
      throw error;
    }
  }

  /**
   * Retrieves firmware version info and OTA update state.
   * Requires REST API v2 (firmware 3.1.0+).
   * @returns {Promise<{installed_version: string, latest_fw: string, latest_beta_fw: string,
   *   fw_update_available: boolean, state: string, progress: number}>}
   * @throws {Error} If the request fails, returns invalid data, or v2 is disabled
   */
  async getOtaInfo() {
    if (!this.webclient._useApiV2) {
      throw new Error('getOtaInfo requires REST API v2 (firmware 3.1.0+)');
    }

    try {
      const response = await this.webclient.get(NRGWatchApi.ENDPOINTS.V2_OTA);
      const result = JSON.parse(response);
      if (result.status === 'success' && result.data) {
        return result.data.ota ?? result.data;
      }
      throw new Error('Invalid v2 response format: missing ota data');
    } catch (error) {
      this.homey?.error('Failed to get OTA info:', error.message);
      throw error;
    }
  }

  /**
   * Sets the outside temperature on a WPU (heat pump) unit.
   * Requires REST API v2.
   * @param {number} temp - Temperature in Celsius (-100 to 100)
   * @returns {Promise<boolean>} True if successful
   * @throws {Error} If the request fails or the device rejects the value
   */
  async setOutsideTemp(temp) {
    if (temp < -100 || temp > 100) {
      throw new Error(`Invalid outside temperature: ${temp}. Must be between -100 and 100`);
    }

    this.homey?.log(`Setting WPU outside temperature to ${temp}°C`);

    try {
      const response = await this.webclient.post(NRGWatchApi.ENDPOINTS.V2_WPU_OUTSIDE_TEMP, { temp });
      if (this._isSuccessResponse(response)) return true;
      throw new Error('Device did not confirm outside temperature change');
    } catch (error) {
      this.homey?.error('Failed to set outside temperature:', error.message);
      throw error;
    }
  }
