...
class WebClient extends BaseClient {
  ...
  async get(resource, params = {}) {
    ...
    return new Promise((resolve, reject) => {
      ...
      // destroy(error) surfaces the timeout through the 'error' handler above,
      // so it is logged and rejected exactly once
  ...
  async testConnection(ipAddress, userName = null, passWord = null) {
    ...
    if (normalizedUserName) {
      ...
      params.password = normalizedPassWord;
    }
    ...
    return new Promise((resolve, reject) => {
      ...
      req.setTimeout(WebClient.DEFAULTS.TIMEOUT, () => {
        req.destroy(new Error('Connection test timeout'));
      });
